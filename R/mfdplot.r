#--------------------------------------------------------------
#' Multivariate Functional Data Visualization Tools Using Plotly
#'
#' This is a plotting method for univariate or multivariate functional time series (\code{\link{mfd}}). This method is designed to help the user visualize
#' \code{\link{mfd}} data using a variety of techniques that use plotly.
#'
#' @param x An object of class \code{\link{mfd}}.
#' @param vars A numeric specifying which variables in the mfd to plot. The default is to plot all variables in succession. Note as well that variable indices may be repeated.
#' @param types A tuple of strings specifying the types of plots to be displayed where possible types for mfd variables observed over a one-dimensional domain are:
#' \itemize{
#' \item \code{"line"} plot the \code{\link{mfd}} elements in a line plot (default)
#' \item \code{"heatmap"} plot the \code{\link{mfd}} elements in a heat map which can be used for variables observed over one or two-dimensional domains
#' \item \code{"3Dsurface"} plot the \code{\link{mfd}} elements as a surface
#' \item \code{"3Dline"} plot the \code{\link{mfd}} elements in a three-dimensional line plot.
#' }
#' The current plot type supported for mfd variables observed over a two-dimensional domain is \code{"heatmap"}. Also note that
...
#'
#' @export
plot.mfd <- function(x, vars = NULL, types = NULL, subplot = TRUE, mains = NULL, ylabels = NULL, xlabels = NULL, tlabels = NULL, zlabels = NULL, ...) {
  p <- length(x@C)
  N <- ncol(x@C[[1]])
  time <- colnames(x@C[[1]])
  count_twod <- 0
