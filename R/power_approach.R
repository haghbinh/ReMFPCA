power_algo_fun <-  function(data,n,lambda = NULL,center = TRUE,alpha_orth,lambda_type = "variable"){
  library(expm)
  p = data$nvar
  #penalty = ReMFPCA:::pen_fun(data,type = "basispen")
  penalty = ReMFPCA:::pen_fun(data,type = "coefpen")
  B =c()
  if (is.null(lambda)) {
    for (i in 1:p) {
      lambda = c(lambda,list(2^seq(-20,20,length.out=10)))
    }
  }
  ...
  if (alpha_orth == FALSE) {
    ...
    for (i in 1:n) {
      ...
      for (j in 1:count){
        ...
        if (lambda_type == "variable") {
          if (i == 1) {
            I[[j]] =  ReMFPCA:::I_alpha(data,lambda[j,])
            D[[j]] = I[[j]]%*%penalty
            ...
            I[[i]] =  ReMFPCA:::I_alpha(data,lambda[[i]])
            D[[i]] = I[[i]]%*%penalty
      ...
      sigma[i] = norm(B%*%G%*%b,type = '2')/sqrt(data$nobs-1)
    }
    ...
    sigma = sqrt(diag(t(temp2)%*%(temp2)))/sqrt(data$nobs-1)
